                    ...
                    <div class="cart-box">
                        ...
                        <div id="orderForm" name="orderForm" url="">
                            <div class="form-box address-info">
                                <el-form :model="order" :rules="rules" ref="orderRef" label-width="150px">
...
components: { VDistpicker },
  ...
  methods:{
      getGoodsListData(){
          ...
          this.$axios.get(`site/validate/order/getgoodslist/${this.$route.query.ids}`).then(response=>{
              ...
              response.data.message.forEach(item=>{
                //   console.log(localGoods[item.id]);
                console.log(localGoods[item.id])
                    if(!localGoods[item.id])return 
    ...
    //返回购物车
    goBack(){
        // this.$router.push({path:'/toshopcart'})
        this.$router.go(-1)
    },
    //下单操作
    goToOrder(){
